...
public class VisitCtrl {
		...
		if(!visits.isEmpty()) {
			for(Visit v : visits) {
				if(v.getEmployee().getId() != visit.getEmployee().getId() && v.getStartTime() != visit.getStartTime()) {
					if(v.getDependent().getId() == dependent.getId()) {
						available = false;
					}
					else if(v.getStartTime() != visit.getStartTime() && v.getVisitEnd() != visit.getStartTime()) {
						available = true;
					}
				}
			}
	...
	//public Visit findByEmpAndTime(String date, int hour, int minutes, Employee employee) throws SQLException {
	//	LocalTime time = LocalTime.of(hour, minutes);
	//	return dbVisit.findByEmpAndTime(employee, date, time);
	//}
